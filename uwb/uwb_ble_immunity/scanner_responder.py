...
def scan_result(result):
    ...
    scanner.stop()
    conn = canvas_ble.connect(
        result.addr, canvas_ble.PHY_1M, ble_connected, ble_disconnected)
...
def start_ranging():
    ...
    session = canvas_uwb.session_new(
        UWB_SESSION_ID, canvas_uwb.ROLE_RESPONDER)
    session.set_local_addr(UWB_RESPONDER_ADDR)
    session.set_peer_addr(UWB_INITIATOR_ADDR)
...
def start_scanning():
    ...
    scanner.filter_add(scanner.FILTER_UUID, MY_UUID)
    scanner.start(1)
